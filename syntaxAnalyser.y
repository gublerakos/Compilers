...
program             :header declarations subprograms                                    {scope++;}
                    ...
                    T_DOT                                                               {hashtbl_get(hashtbl, scope); scope--; printf("Program ended with T_DOT.\n");}
                    ;
...
constant_defs       :constant_defs T_SEMI T_ID                                          {hashtbl_insert(hashtbl, $3, NULL, scope); }
                    ...
                    |constant_defs error T_ID T_EQU expression                          {yyerror("MISSING SEMI!\n"); yyerrok;}
                    ;
...
type_defs           :type_defs T_SEMI T_ID                                              {hashtbl_insert(hashtbl, $3, NULL, scope); printf("TYPE_DEFS -> type_defs T_SEMI T_ID\n");}
                    ...
                    |T_ID T_EQU type_def                                                {hashtbl_insert(hashtbl, $2, NULL, scope); printf("TYPE_DEFS -> T_ID T_EQU type_def\n");}
                    |type_defs error T_ID T_EQU type_def                                {yyerror("MISSING SEMI!"); yyerrok;}
                    ;
...
fields              :fields T_SEMI field                                                {printf("FIELDS -> fields T_SEMI field\n");}
                    ...
                    |fields error field                                                 {yyerror("MISSING SEMI!"); yyerrok;}
                    ;
...
if_statement        :T_IF expression T_THEN                                             {scope++;}
                    statement if_tail
                    |T_IF expression error statement if_tail                            {yyerror("IF WITHOUT THEN!\n"); yyerrok;}
                    ;
if_tail             :T_ELSE statement                                                   {hashtbl_get(hashtbl, scope); scope--; printf("IF_TAIL -> T_ELSE statement\n");}
                    |%prec LOWER_THAN_ELSE                                              {hashtbl_get(hashtbl, scope); scope--; printf("IF_TAIL -> empty\n");}
                    ;
while_statement     :T_WHILE expression T_DO                                            {scope++;}
                    statement                                                           {hashtbl_get(hashtbl, scope); scope--; printf("WHILE_STATEMENT -> T_WHILE expression T_DO statement\n");}
                    ;
for_statement       :T_FOR                                                              {scope++;}
                    T_ID                                                                {hashtbl_insert(hashtbl, $1, NULL, scope);}
                    T_ASSIGN iter_space T_DO statement                                  {hashtbl_get(hashtbl, scope); scope--; printf("FOR_STATEMENT -> T_FOR T_ID T_ASSIGN iter_space T_DO statement\n");}
                    ;
iter_space          :expression T_TO expression                                         {printf("ITER_SPACE -> expression T_TO expression\n");}
                    ...
                    ;
with_statement      :T_WITH                                                             {scope++;}
                    variable T_DO statement                                             {printf("WITH_STATEMENT -> T_WITH variable T_DO statement\n"); scope--;}
                    ;
...
io_statement        :T_READ T_LPAREN read_list T_RPAREN                                 {printf("IO_STATEMENT -> T_READ T_LPAREN read_list T_RPAREN\n");}
                    ...
                    |T_READ T_LPAREN read_list error                                    {yyerror("MISSING RPAREN!\n"); yyerrok;}
                    |T_WRITE T_LPAREN write_list error                                  {yyerror("MISSING RPAREN!\n"); yyerrok;}
                    ;
